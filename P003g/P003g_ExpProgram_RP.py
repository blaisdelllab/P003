...
# -*- coding: utf-8 -*-
"""
Created on Fri May  1 17:22:04 2026

Last updated: 05-01-26

@author: megancady

P003g: Yoked Spatial Variability Study

This experiment tests whether the positive relationship between reinforcement
and spatial variability observed under RR20 omission schedules depends on the
bird's own behavior producing food, or whether it can be explained by the
pattern of food delivery alone. In the Master phase, pigeons complete RR20
Instrumental (INS), Omission (OMS), and Pavlovian (PAV) trials with food
delivery determined by the programmed contingency. In the Yoked phase, pigeons
will later receive food on the same trial-by-trial schedule from their own
Master phase, but independently of their current responding. 
...
class ExperimenterControlPanel(object):
    ...
    def __init__(self):
        ...
        if operant_box_version:
            ...
            self.data_folder = "P003g_data" # The folder within Documents where subject data is kept
        ...
        self.control_window.title("P003g Control Panel")
        ##  Next, setup variables within the control panel
        # Subject ID
        self.pigeon_name_list = ["Herriot", "Peach", "Wario",
                                 "Kurt", "Hendrix", "Iggy",
                                 "Hawthorne"]
        ...
        # Exp phases
        self.experimental_phase_titles = ["Master", "Yoked"]
        ...
        if self.subject_ID_variable.get() in self.pigeon_name_list:
            if self.exp_phase_variable.get() != "Select":
                ...
                self.MS = MainScreen(
                    ...
                    self.experimental_phase_titles.index(self.exp_phase_variable.get()) # Exp Phase number (0 through 1; 0-Master, 1-Yoked)
                    )
            else:
...
class MainScreen(object):
    ...
    def __init__(self, subject_ID, record_data, data_folder_directory,
        ...
        self.exp_phase_name = exp_phase_name # e.g., "Master"
        self.exp_phase_num = exp_phase_num # e.g., 0 (0 through 1; 0-Master, 1-Yoked)
        ...
        self.root = Toplevel()
        self.root.title(f"P003g {self.exp_phase_name}: Spatial Variability") # this is the title of the window
    ...
    def place_birds_in_box(self):
        ...
        def first_ITI(event):
            ...
            # Next, pick out the specific stimuli images for that specific 
            ...
            self.stimulus_assignments_dict["INS"] = all_stimulus_assignments_dict["RR20_INS"]
            self.stimulus_assignments_dict["OMS"] = all_stimulus_assignments_dict["RR20_OMS"]
            self.stimulus_assignments_dict["PAV"] = all_stimulus_assignments_dict["PAV"]

            # Now that each bird has a stimulus image assigned to each
            # condition, we can load the actual image files. The images need to
            # be stored in the P003g/stimuli folder on the desktop.
            self.stimulus_image_dict = {}
            for trial_type, stimulus_file in self.stimulus_assignments_dict.items():
                stimulus_path = os_path.join(stimuli_folder_path, stimulus_file)
                stimulus_image = Image.open(stimulus_path)
                stimulus_image = stimulus_image.resize((192, 192), Image.LANCZOS)
                self.stimulus_image_dict[trial_type] = ImageTk.PhotoImage(stimulus_image)

            # Once we have the three stimuli images for each bird and phase,
    ...
    def build_keys(self):
        ...
        self.mastercanvas.create_image(512, 384,
                                       image = self.stimulus_image_dict[self.trial_type],
                                       tag = "key")
    ...
    def write_data(self, event, outcome):
            ...
            self.session_data_frame.append([
                ...
                self.exp_phase_name, # Phase name (e.g., Master)
    ...
    def write_comp_data(self, SessionEnded):
        ...
        if self.record_data : # If experimenter has choosen to automatically record data in seperate sheet:
            myFile_loc = f"{self.data_folder_directory}/{self.subject_ID}/{self.subject_ID}_{self.start_time.strftime('%Y-%m-%d_%H.%M.%S')}_P003g_data.csv" # location of written .csv
